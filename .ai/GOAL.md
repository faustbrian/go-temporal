...
`github.com/faustbrian/temporal` while using explicit immutable values, typed
...
- `go-clock` owns time sources and timers; `go-calendar` owns civil arithmetic,
...
- Snapping to civil boundaries requires an explicit `go-calendar` adapter,
...
- Optional integration using `go-calendar.Date` for bounded civil date ranges.
...
- Applying a time-of-day to a civil date requires explicit location and DST
  resolution through `go-calendar`.
...
- `dateperiod`: optional `go-calendar.Date` periods and sequences.
