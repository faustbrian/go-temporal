...
| `DatePoint` | `time.Time` or `go-calendar.Date` | instant and civil date are distinct |
...
`go-calendar`. End-of-day conversion becomes a next-boundary exclusive instant
range, preserving DST behavior.

PHP snap helpers become `instant.Snap` or `Period.SnapOutward`; callers provide
the unit, direction, location, and `go-calendar` gap/fold policy. PHP
