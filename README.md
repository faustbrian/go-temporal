...
`go-temporal` is immutable temporal algebra for Go. It provides bounded instant
...
The module is a Go-native successor to `github.com/faustbrian/temporal`. It
...
Date arithmetic delegates to `go-calendar`. Conversion uses the next civil
...
- `dateperiod`: bounded `go-calendar.Date` periods and normalized sets.
...
- `temporalvalidation`: deterministic `go-validation` rules.
- `temporalconfig`: atomic `go-config` text wrappers.
